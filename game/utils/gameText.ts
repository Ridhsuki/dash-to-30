...
function normalizeRoast(value: unknown): string {
  ...
  return raw
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 180) || DEFAULT_CONFIG.roast;
}

function fillList(items: string[], fallback: string[], targetLength: number): string[] {
  const result = [...items];
...
export function normalizeAiConfig(input: unknown): GameAiConfig {
  const value = input && typeof input === 'object' ? input as Partial<GameAiConfig> : {};
...
export function compactEntityLabel(
  ...
  const usefulWords = words.filter((word) => !STOP_WORDS.has(word.toLowerCase()));
  const selectedWords = usefulWords.length > 0 ? usefulWords : words;

  const maxLength = kind === 'need' ? 13 : 12;
  const shortLabel = limitText(selectedWords.slice(0, 2).join(' ').toUpperCase(), maxLength);
