...
export default function ParallaxGameBackground() {
  ...
  const farOffset = isReduced
    ? { x: 0, y: 0 }
    : { x: coords.x * -4, y: coords.y * -2 };
  // Layer 2 (Mid-ground decorative city) moves moderately
  const midOffset = isReduced
    ? { x: 0, y: 0 }
    : { x: coords.x * -10, y: coords.y * -5 };
    ...
    <div className="absolute inset-0 pointer-events-none overflow-hidden z-0 select-none bg-[#DFF4FF]">
      ...
      >
        ...
        <div className="absolute bottom-8 left-[12%] w-20 h-28 bg-[#CDEFFF]/70 rounded-t-xl border-t-4 border-x-4 border-[#BFEDFF]">
          {/* Green ATM symbol */}
          <div className="absolute top-3 left-1/2 -translate-x-1/2 text-[9px] font-bold text-[#6FD08C] bg-white px-1.5 py-0.5 rounded border border-[#6FD08C]/40">
            ATM
          </div>
        </div>
      ...
      <div className="absolute inset-0 z-10 w-full h-full">
        {INITIAL_PARTICLES.map((p) => {
          ...
          if (p.type === "need") {
            customClass +=
              "text-[9px] font-pixel tracking-wider px-2.5 py-1 border border-[#6FD08C]/30 bg-white/90 rounded-full text-[#6FD08C] font-semibold whitespace-nowrap shadow-sm";
          } else if (p.type === "want") {
            customClass +=
              "text-[9px] font-pixel tracking-wider px-2.5 py-1 border border-[#FF7AA2]/30 bg-white/95 rounded-full text-[#FF7AA2] font-semibold whitespace-nowrap shadow-sm";
          } else if (p.type === "paylater") {
            customClass +=
              "text-[9px] font-pixel tracking-wider px-2.5 py-1 border border-[#9B8CFF]/30 bg-white/95 rounded-full text-[#9B8CFF] font-semibold whitespace-nowrap shadow-sm";
          } else if (p.type === "danger") {
            customClass +=
              "text-[9px] font-mono tracking-wider px-2 py-0.5 border border-[#FF6B6B]/30 bg-white/95 rounded text-[#FF6B6B] font-bold whitespace-nowrap shadow-sm";
          } else {
