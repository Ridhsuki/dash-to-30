    ...
    const timer = setTimeout(() => {
      ...
      try {
        ...
        unsubscribe = onSnapshot(
          ...
          (error) => {
            console.warn('Firestore onSnapshot error, falling back to local demo scoring:', error);
            setScores(FALLBACK_SCORES);
            setUsingFallback(true);
            setLoading(false);
          }
        ...
        console.warn('Real-time listener setup caught error, using local demo scoring:', e);
        setScores(FALLBACK_SCORES);
        setUsingFallback(true);
        setLoading(false);
      }
    ...
    >
      ...
      >
        ...
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 w-[70%] h-5 bg-[#FFF1C7] border-2 border-[#8B5E3C] border-b-0 rounded-t-lg flex items-center justify-between px-3 text-[8px] text-[#4A3A2A] font-mono tracking-widest font-bold">
          <span className="flex items-center gap-1"><Trophy className="w-2.5 h-2.5 text-[#FF9F1C]" /> LEADERBOARD</span>
        ...
        {usingFallback && (
          <div className="mb-3.5 bg-[#FFF2DA] border border-[#FF9F1C]/40 rounded-xl p-2 sm:p-2.5 flex items-start gap-2 text-left shrink-0">
            ...
            <div className="text-[8px] sm:text-[9px] font-mono text-[#4A3A2A]/80 leading-normal font-bold uppercase py-0.5">
              <span>SHOWING DEMO RANKINGS. INTEGRATE CLOUD DB TO RENDER LIVE SUBMISSIONS!</span>
            </div>
          </div>
        )}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`w-6 h-6 rounded-lg border-2 border-[#8B5E3C] flex items-center justify-center font-pixel text-xs ${rank <= 3 ? 'bg-white' : 'bg-[#FFF1C7]/50'} ${rankTextColor}`}>
                      {rank}
                    </span>
                    <div className="flex flex-col text-left">
                      ...
                      <span className="text-[7px] font-mono text-[#4A3A2A]/50 font-bold uppercase tracking-wider flex items-center gap-1 mt-0.5">
                        <Calendar className="w-2.5 h-2.5" /> Survived: {entry.survivalDays}/30 Days
                      </span>
                    </div>
                  </div>
